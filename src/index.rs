...
// One embedded SPA file plus its precompressed variants (generated by
// `build.rs` at compile time, so serving them costs no runtime CPU).
// `embedded_get` returns this; see the generated module below.
// Fields are private: only this module constructs it, callers just pass it to
// the server internals.
pub struct EmbeddedAsset {
    /// Raw file bytes (identity encoding fallback).
    data: &'static [u8],
    /// Content-Type derived from the real asset filename.
    mime: &'static str,
    /// gzip bytes, when compressing shrank the file.
    gzip: Option<&'static [u8]>,
    /// brotli bytes, when compressing shrank the file.
    br: Option<&'static [u8]>,
}
...
fn serve_spa_fallback(path: &str, accept_encoding: &str) -> Response<RespBody> {
...
mod tests {
    ...
    fn test_accepts_encoding() {
        ...
        assert!(!accepts_encoding("gzip, deflate, br", "zstd"));
        assert!(accepts_encoding("br;q=0.8, gzip;q=0.5", "br"));
        assert!(accepts_encoding("BR", "br"));
        // Explicit `q=0` refusal wins, even with a wildcard present.
    ...
    fn test_asset(data: &'static [u8]) -> EmbeddedAsset {
        EmbeddedAsset {
            data,
            mime: "text/javascript; charset=utf-8",
            gzip: Some(b"gz-bytes" as &[u8]),
            br: Some(b"br-bytes" as &[u8]),
        }
    }

    #[tokio::test]
    async fn test_asset_response_prefers_brotli() {
        let resp = asset_response(
            test_asset(b"raw-bytes" as &[u8]),
        ...
        assert_eq!(body_bytes(resp).await, b"br-bytes");
    }

    #[tokio::test]
    async fn test_asset_response_gzip_when_no_brotli_offered() {
        let resp = asset_response(
            test_asset(b"raw-bytes" as &[u8]),
            "/assets/app-abc123.js",
            "gzip",
    ...
    #[tokio::test]
    async fn test_asset_response_identity_without_accept_encoding() {
        let resp = asset_response(
            test_asset(b"raw-bytes" as &[u8]),
            "/assets/app-abc123.js",
            "",
        ...
        // Vary is still advertised: the representation depends on the header.
        assert_eq!(resp.headers().get("vary").unwrap(), "accept-encoding");
        assert_eq!(body_bytes(resp).await, b"raw-bytes");
    }

    #[tokio::test]
    async fn test_asset_response_entry_html_is_not_cached() {
        // `index.html` is unhashed: clients must revalidate so deploys land.
        let resp = asset_response(test_asset(b"<html>" as &[u8]), "/index.html", "br");
        assert_eq!(resp.headers().get("cache-control").unwrap(), "no-cache");
        assert_eq!(body_bytes(resp).await, b"br-bytes");
    }

    #[tokio::test]
    async fn test_asset_response_uncompressed_asset_has_no_vary() {
        ...
        let resp = asset_response(asset, "/assets/font-abc.woff2", "gzip, br");
        assert!(resp.headers().get("content-encoding").is_none());
        assert!(resp.headers().get("vary").is_none());
        assert_eq!(body_bytes(resp).await, b"raw");
    }
