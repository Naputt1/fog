...
// returning `None`, so the crate still compiles and the server falls back to
// its generated service-directory page.
...
/// Serves a non-API, non-`/logs` request from the embedded SPA. Returns an
/// exact static asset when one exists (correct content-type), otherwise the SPA
/// `index.html` so client-side routes resolve. When no SPA is embedded, falls
/// back to the generated service-directory page (the historical behavior).
fn serve_spa_fallback(
    cfg: &RouterConfig,
    path: &str,
...
/// Builds a reachable URL for a service entry, matching the directory page's
/// logic: HTTP services use `{scheme}://{hostname}/`; raw-TCP services exposed
/// via `fog.expose` use `http://{hostname}:{raw_port}/`.
...
fn load_runtime_config() -> crate::config::Config {
    let home = std::env::var("HOME").unwrap_or_default();
    let candidates = [
...
mod tests {
    ...
    #[test]
    fn test_spa_root_never_served_as_octet_stream() {
        // Regression for the "browser downloads the page" bug: `/` must be
        // served as text/html — whether from the embedded SPA's index.html or
        // from the generated service-directory fallback — and never as
        // `application/octet-stream` (which makes browsers download the file).
        let cfg = RouterConfig {
            shared_network: "fog-router".to_string(),
            ..RouterConfig::default()
        };
        let resp = serve_spa_fallback(&cfg, "/", None);
        let ct = resp
            .headers()
            .get("content-type")
            .and_then(|v| v.to_str().ok())
        ...
        assert!(
            ...
            "expected text/html for /, got {ct:?}"
        );
    }

    #[test]
    fn test_spa_client_route_served_as_text_html() {
        // A client-side route with no matching asset falls back to the SPA
        // entry point and must still be served as text/html (not octet-stream).
        let cfg = RouterConfig {
            shared_network: "fog-router".to_string(),
            ..RouterConfig::default()
        };
        let resp = serve_spa_fallback(&cfg, "/some/client/route", None);
        let ct = resp
            .headers()
            .get("content-type")
            .and_then(|v| v.to_str().ok())
